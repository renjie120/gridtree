...
(function($) {
	$.addGt = function(t, p) {
		...
		p = $.extend({
					...
					title : '',
					exchangeColor : true,
					closeImg : appPath+'gridTree/images/minus.gif',
					openImg : appPath+'gridTree/images/plus.gif',
					lazyLoadImg : appPath+'gridTree/images/plus2.gif',
					blankImg : appPath+'gridTree/images/blank.gif',
					noparentImg : appPath+'gridTree/images/tv-item.gif',
					lastLeafImg : appPath+'gridTree/images/tv-item-last.gif',
					morePageImg : appPath+'gridTree/images/morePage.gif',
					...
					handleCheck : null,
					data : null,
					dynamicColumn : null,// 动态加载数据时可以根据这个属性设置是否是文件夹!前提：必须有data
					setArguments : null,// 超级表格的时候需要传递到后台的行对象里面的参数数组,前提必须设置data和dynamicColumn!函数返回对象类型，将要传递到后台的参数以对象形式返回
					lazy : false,
					lazyLoadUrl : null,
					handler : null,
					leafColumn : null,
		...
		var g = {
			...
			_resetBasePageBtns : function() {
				g._d('_resetBasePageBtns');
				g._resetPageBtns(pagingInfo, "_firstPageBtn", "_prePageBtn",
						"_lastPageBtn", "_nextPageBtn");
				if (pagingInfo.pagesCount == 1 || pagingInfo.pagesCount == 0) {
					if (pagingInfo.pagesCount == 0) {
						$('#_toPageBtn', elct).removeClass("nextPage")
								.addClass("disFirstPage")
								.attr("disabled", true);
					}
				}
			},
			_resetLazyPageBtns : function(pid) {
					g._d('_resetLazyPageBtns');
				var ll = _lazypgInfoMap.get(pid);
				g._resetAllLazyPageBtns(ll, "_fstbtn_" + pid, "_prebtn_" + pid,
						"_lstbtn_" + pid, "_nxtbtn_" + pid);
			},
			...
			_makeTable : function(data) {
				...
				g._initPageInfo();
				var tree = g._getTree();
				g._content_div.append(tree);
				g._setLastLeaf();
				var _title_width = $('#_title_Div th', elct);
				var __len = _title_width.size();
				var hasAdded = false;
				var isthere = 0;
				$('#_changePageSizeSel option', elct).each(function(i) {
				...
				if (!hasAdded) {
					if (isthere != 0)
						$('#_changePageSizeSel option:eq(' + isthere + ')',
								elct).after("<option value='" + p.pageSize
								+ "' selected='selected'>" + p.pageSize
								+ "</option>");
					else {
						$('#_changePageSizeSel', elct)
								.prepend("<option value='" + p.pageSize
										+ "' selected='selected'>" + p.pageSize
										+ "</option>");
				...
				if (p.pageBar) {
					var pgdiv = $('#_pagebar_pagesdiv', elct);
					var a_btns = pgdiv.find('a');
					$(a_btns[0], elct).bind('click', function() {
								g._toPage('first');
							});
					$(a_btns[1], elct).bind('click', function() {
								g._toPage('pre');
							});
					$(a_btns[2], elct).bind('click', function() {
								g._toPage('next');
							});
					$(a_btns[3], elct).bind('click', function() {
								g._toPage('last');
							});
					$(pgdiv.find('.goto'), elct).bind('click', function() {
								g._toPage('any');
							});
					$('#_changePageSizeSel', elct).bind('change', function() {
								g._reMakeTable(this.value);
							});
					$('#_changePageSizeSel option[value=' + p.pageSize + ']')
			...
			_analyseData : function(msg) {
				...
				if (!p.lazy) {
					...
					firstLevelParentIds = removeArrayFromOtherArray(parents,
							nodeMap.keys());
					parents = removeArrayFromOtherArray(parents,
							firstLevelParentIds);
					for (var ii = 0; ii < firstLevelParentIds.length; ii++) {
						firstLevelNodes = firstLevelNodes
								.concat(parentToChildMap
			...
			setDisabled : function(val) {
				if (val) {
					$('input,button,select,a', elct).attr('disabled', 'true');
					$('a:not(.pgb)', elct).bind('click', function(eee) {
					...
					$('a,input:not([userSetDisabled=disabled]),button:not([userSetDisabled=disabled]),select:not([userSetDisabled=disabled])')
							.removeAttr('disabled');
					$('a:not(.pgb)', elct).unbind('click');
					g._resetPageBtns(pagingInfo);
				}

			},
			getSelectedRow : function() {
				return $('#' + _lastSelectRowId, elct)[0];
			...
			appendChild : function(parentId, jsonStr) {
				...
				if (!p.lazy) {
					...
					var parentNode = $('#_node' + parentId, elct)[0];
					var img = $('#img_' + parentId, elct)[0];
			...
			_reload : function() {
				...
				$.ajax({
							...
							success : function(msg) {
								if (p.cnsoleJsonStr)
									msg = p.cnsoleJsonStr(msg);
								var tb = $('#' + p.tableId, elct)[0];
								var o = new Date();
								g._newPagingServerMakeTable(tb, msg);
								g._showMsg(0);
								g._setLastLeaf();
								mcel.html(["当前第", pagingInfo.currentPage,
										"页/总共", pagingInfo.pagesCount, "页"]
										.join(''));
								var gotime = new Date() - o;
			...
			dynamicAdd : function(parentId, parentNode, imgNode, childNodeStrs,
					prebotherindex) {
				g._d('dynamicAdd');
				if (!imgNode) {
				...
				$(imgNode).unbind('click').bind('click', function(e) {
							g.closeChildrenTable('img_' + parentId, imgNode, e);
						});
				var level = parseInt(parentNode.getAttribute('_node_level'))
						+ 1; 
				var __row_num = parentNode.getAttribute('rownum') == null
						? parentNode.getAttribute('_node_num')
						: parentNode.getAttribute('rownum');
				var rownumstr = (__row_num + "").split('.');
				var startIndex = 1;
				...
				if (!(p.lazyPage || p.lazyMorePage)) {
					...
					if (p.allDataInfoWithLazyPageInfo) {
						if (p.allDataInfoWithLazyPageInfo.data)
							datas = p.allDataInfoWithLazyPageInfo.data;
						else {
				...
				for (var i = 0; i <= datas.length - 1; i++) { 
					...
					var newRow = g._addOneLazyRowByData(startIndex, datas[i],
							level, i + 1 + prebotherindex * 1, null);
					if (p.lazyPage && (i == datas.length - 1)
							&& !p._dynamicAddByUse) {
						g._d(394)
						newRow.setAttribute('_lazy_paging_pId', parentId);
						$('td[_td_pro]:eq(0) img:eq(' + (level - 2) + ')',
								newRow).attr('src', p.morePageImg).mouseover(
								function() {
									$(this).attr({
												'id' : '_lazypagingimg_'
														+ parentId
									...
									g._showLazyPagingDiv(parentId, this.id,
											true, 1);
								});

					} else if (p.lazyMorePage && (i == datas.length - 1)
							&& !p._dynamicAddByUse) {
						g._d(406)
						$(
								'tr[_lazy_paging_pId=' + parentId
										+ ']:last img:eq(' + (level - 2) + ')',
								elct).attr('src', p.blankImg);
						$('tr[_lazy_paging_pId=' + parentId + ']', elct)
								.removeAttr('_lazy_paging_pId');
						$('#_lazypagingimg_' + parentId, elct).removeAttr('id')
								.unbind('mouseover').unbind('click')
								.unbind('mouseout');
						newRow.setAttribute('_lazy_paging_pId', parentId);
						var lazypaging_info = _lazypgInfoMap.get(parentId);
						if (lazypaging_info.currentPage
								* lazypaging_info.pageSize < lazypaging_info.allCount)
							$('td[_td_pro]:eq(0) img:eq(' + (level - 2) + ')',
									newRow).attr({
										'src' : p.morePageImg,
										'title' : '点击查看更多'
				...
				if (p.lazyPage || (p.lazyMorePage && p._dynamicAddByUse)) {
					...
					if ($('tr[_node_parent=_node' + parentId + ']', elct)
							.size() > 0) {
						// $('tr[_node_parent=_node' + parentId +
						// ']:last',elct).after(temp);
						$(parentNode).after(temp);
					} else
						$(parentNode).after(temp).attr('_expaned', 'true');
			...
			openChildrenTable : function(imgid, node, e) {
				g._d('openChildrenTable'); 
				var _id = imgid.replace('img_', '').replace('_node', '');
				// var _i = $('#img_' + _id,elct); g._d(460);
				var _i = $(node);
				if (!_i)
					return false; 
				var img = _i[0];
				// var pN = $('#_node' + _id+":visible",elct);
				// //这里修改了查找nodeid的算法.
				var pN = $(node).parent().parent().parent(); 
				if (pN.size() < 1 || pN.attr('_open') == 'true') {
					return false;
				} 
				// 如果是懒加载或者是超级表格模式...
				if (p.lazy || g.superGrid) { 
					var param = {};
					if (p.lazy) {
						param = {
							pId : _id
						...
						param = p.setArguments(node);
					}
					// 防止出现多次懒加载加载显示重复的表格数据.
					...
					if (pN.attr('_expaned') != 'true') {
									...
									success : function(msg) {
										if (msg != null && msg != "") {
											...
											try {
												if (p.lazyPage
														|| p.lazyMorePage) {
													g._initLazyPageInfo(_id,
															'父亲节点id', msg);
													g.dynamicAdd(_id, pN[0],
															img, msg);
												...
												if (p.onLazyLoadSuccess)
													p.onLazyLoadSuccess(elct);
						...
						$('tr[_node_parent=_node' + _id + ']', elct).show();
						$(img, elct).unbind('click').attr('src', p.closeImg)
								.bind('click', function(ee) {
											g.closeChildrenTable('img_' + _id,
													img, ee)
										});
					}
					if (p.exchangeColor)
					...
					g._d(544);
					$(img, elct).unbind('click').attr('src', p.closeImg).bind(
							'click', function(ee) {
								g.closeChildrenTable(imgid, node, ee)
							});
					$('tr[_node_parent=_node' + _id + ']', elct).each(
							function(i, ee) {
								var $this = $(this);
			...
			_hideByPath : function(ppth, e) { 
				g._d("_hideByPath中ppth=" + ppth + "--"
						+ $('tr[_node_path^=' + ppth + ']', elct).size());
				$('tr[_node_path^=' + ppth + ']', elct).each(function() {
					var $this = $(this);
					var id = $this.attr('id').replace('_node', '');
					if ($this.attr('_node_isparent') == '1') {
						$('#img_' + id, elct).attr('src', p.openImg)
								.unbind('click').bind('click', function(ee) {
											g.openChildrenTable(id, this, ee);
										});
			...
			_newHideByPath : function(node, id) {
				g._d("_newHideByPath");
				$(node).nextAll('tr').each(function() {
					var $this = $(this);
					var _id = $this.attr('id').replace('_node', '');
					var _path = $this.attr('_node_parent');
					if (_path.indexOf(id) != -1) {
						$this.hide();
					} else {
						// 遇到第一个不是孩子树里面的节点，就退出循环.
						return false;
					}
					// 如果是父亲节点，设置打开节点的方法，并修改图标样式.
					if ($this.attr('_node_isparent') == '1') {
						$('#img_' + _id, elct).attr('src', p.openImg)
								.unbind('click').bind('click', function(ee) {
											g.openChildrenTable(_id, this, ee);
										});
						$this.attr('_open', 'false');
					}
				});
			...
			closeChildrenTable : function(imgid, node, e) { 
				var _id = imgid.replace('img_', '').replace('_node', '');
				var _i = $('#img_' + _id, elct);
				g._d("567---closeChildrenTable()---关闭子节点:" + _id);
				if (_i.size() < 1)
					return false;
				var img = _i[0];
				img.src = p.openImg;
				$(img, elct).unbind('click').bind('click', function(ee) {
							g.openChildrenTable(imgid, node, ee);
						});
				var pN = $('#_node' + _id, elct);
				pN.attr('_open', 'false');
				g._newHideByPath(pN, '_node' + _id);
				if (p.exchangeColor)
					g._setColor();
			},
			select : function(nodeId) {
				$(
						'input[name=_chks][value=' + nodeId
								+ ']:not([userSetDisabled=disabled])', elct)
						.attr('checked', true);
			...
			closeAll : function() {
				if (!p.lazy) {
					$('tr[_node_isparent=1]', elct).each(function() {
						var _$this = $(this);
						var id = _$this.attr('id').replace('_node', '');
						_$this.find('img[id=img_' + id + ']').attr('src',
								p.openImg).unbind('click').bind('click',
								function(e) {
									g.openChildrenTable('img_' + id, this, e);
								});
						_$this.attr('_open', 'false');
					});
					$('tr[id^=_node][_node_level!=1]', elct).hide();
				} else {
					g._e('懒加载模式下全部关闭不可用.');
				}
			...
			expandAll : function() {
				g._d("expandAll" );
				if (!p.lazy) {
					$('tr[id^=_node]', elct).show();
					$('tr[_node_isparent=1]', elct).each(function() {
						var _$this = $(this);
						var id = _$this.attr('id').replace('_node', '');
						_$this.find('img[id=img_' + id + ']').attr('src',
								p.closeImg).unbind('click').bind('click',
								function(e) {
									g
											.closeChildrenTable('img_' + id,
													_$this, e);
								});
			...
			_postGetTable : function(u, p) {
				...
				$.ajax({
							...
							error : function(data) {
								if (p.cnsoleJsonStr)
									data.responseText = p
											.cnsoleJsonStr(data.responseText);
			...
			_setLastLeaf : function() { 
				...
				if (lastLeafMap.size() > 0) {
					...
					for (__i = 0; __i < allLeaf.length; __i++) {
						// 为什么下面的allLeaf里面会有非空？？？这是一个bug！
						var _lastLeafId = 'img_'
								+ (allLeaf[__i].replace('_node', ''));
						var src = $('#' + _lastLeafId, elct).attr('src');
						if (src) {
							if (src.indexOf(p.minNoparentImg) != -1) {
								$('#' + _lastLeafId, elct).attr('src',
										p.lastLeafImg);
							}
						}
					}
			...
			_parseLazyData : function(data) {
				...
				for (var i = 0, j = data.length; i < j; i++) {
					...
					if (findInArray(allId, _p) == -1)
						idToNumMap.put(_id, pToNumMap.get(_p));
					else {
						idToNumMap.put(_id, idToNumMap.get(_p) + '.'
										+ pToNumMap.get(_p));
				...
				return {
					'specPids' : removeArrayFromOtherArray(allp,
							removeArrayFromOtherArray(allp, allId)),
			...
			_showLazyTable : function(tableTree, data) {
				...
				for (var i = 0, j = datas.length; i < j; i++) {
					...
					var newRow = g._addOneLazyRowByData(rowCount, oneObj, 1,
							null, specP, _num);
					if ($('tbody', tableTree).size() > 0)
						$(tableTree).children('tbody').append(newRow);
					else
				...
				function addChildByParentNode(parentNode, level) {
					...
					if (_children != null) {
						for (var i = 0; i < _children.length; i++) {
							...
							g._addOneRowByData(tableTree, rowCount, oneObj,
									level);
							rowCount = rowCount + 1;
			...
			_addEventToObj : function(obj, funObj) {
				if (isIE) {
					...
					for (eventName in funObj) {
						...
						obj.addEventListener(eventName.substring(2, len),
								newfun, false);
			...
			_getSelectRowId : function() {
				g._d('_getSelectRowId');
				return _lastSelectRowId;

			},
			_chooseParentNode : function(checkboxDom) {
				g._d('_chooseParentNode');
				var nodeId = checkboxDom.value;
				if (checkboxDom.checked) {
					while (1) {
						var parentId = $('#_node' + nodeId, elct)
								.attr('_node_parent');
						if ($('#' + parentId, elct).size() > 0) {
							nodeId = parentId.replace('_node', '');
							var obj = $('#_chk' + nodeId, elct)[0].firstChild;
			...
			_chooseChildrenNode : function(checkboxDom,id) {
				g._d('_chooseChildrenNode');
			 	 	if (checkboxDom.checked) { 
						g._chooseAllChildrenNode('_node' + checkboxDom.value, true);
					} else{ 
					if(id) nodeId = id; 
					if ($('input[type=checkbox][value='+nodeId+']:checked').size()>0) { 
						g._chooseAllChildrenNode('_node' + nodeId, true);
					}
				} 
			},
			_cancleChildrenNode : function(checkboxDom,id) {
				g._d('_cancleChildrenNode');
				if(checkboxDom.value){
				var nodeId = checkboxDom.value;
				if (!checkboxDom.checked) {
					...
					if(id) nodeId = id;  
					if ($('input[type=checkbox][value='+nodeId+']:checked').size()<1) { 
						g._chooseAllChildrenNode('_node' + nodeId, false);
					}
				}
			...
			_cancelFaher : function(checkboxDom) {
				...
				if (!checkboxDom.checked) {
					while (1 == 1) {
						var parentId = $('#_node' + nodeId, elct)[0]._node_parent;
						if ($('#' + parentId, elct).size() > 0) {
							nodeId = parentId.replace('_node', '');
			...
			_anyChildChoosed : function(parentId) {
				...
				for (var i = 0; i < len; i++) {
					var nodeId = children[i].replace('_node', '');
					if ($('#_chk' + nodeId, elct)[0].firstChild.checked) {
						return true;
			...
			_createContent : function(showTypeDesc, onerow, dataColumn,
					idValue, rowobj) {
				...
				if (setVisible == null || onerow[setVisible] + '' == '1') {
					...
					if (tp == 'textbox') {
						node.innerHTML = ["<input type='text' class='",
								cssName, "'  value='", value, "' name='",
								inputName, "' ", setDisabled,
								" userSetDisabled='", setDisabled, "'/>"]
								.join('');
						...
						if (showTypeDesc.func) {
							node.innerHTML = showTypeDesc.func(onerow, rowobj);
						}
					} else if (tp == 'select') {
						...
						for (var i = 0; i < allvalues.length; i++) {
							if (allvalues[i] == value) {
								ans.push(["<option value='", allvalues[i],
										"' selected>", texts[i], "</option>"]
										.join(''));
						...
						texts = showTypeDesc.texts;
						node.appendChild(createHidden(inputName, inputName,
								value));
						var f = function() {
							if (showTypeDesc.onclick)
								showTypeDesc.onclick();
							$('#' + inputName, elct).val(this.value);

						};
						for (var i = 0; i < allvalues.length; i++) {
							if (value == allvalues[i]) {
								createRadio(node, setDisabled, '', 'rd_'
												+ inputName, allvalues[i], f,
										texts[i], cssName, 'checked');
						...
						for (var i = 0; i < allvalues.length; i++) {
							if (findInArray(chArry, allvalues[i]) != -1) {
								createCheckbox(node, setDisabled, '',
										inputName, allvalues[i], f, texts[i],
										cssName, 'checked');
			...
			_resetAllLazyPageBtns : function(pginfo, fstBtnId, preBtnId,
					lastBtnId, nxtBtnId) {
				var f1t = $('#' + fstBtnId, elct), pre = $('#' + preBtnId, elct), lst = $(
						'#' + lastBtnId, elct), nex = $('#' + nxtBtnId, elct);
				if (pginfo.pagesCount == 1 || pginfo.pagesCount == 0) {
					f1t.removeClass("firstPage").addClass("disFirstPage").attr(
							"disabled", true);
					pre.removeClass("prevPage").addClass("disPrevPage").attr(
							"disabled", true);
					lst.removeClass("lastPage").addClass("disLastPage").attr(
							"disabled", true);
					nex.removeClass("nextPage").addClass("disNextPage").attr(
							"disabled", true);

				} else {
					if (pginfo.currentPage == 1) {
						f1t.removeClass("firstPage").addClass("disFirstPage")
								.attr("disabled", true);
						pre.removeClass("prevPage").addClass("disPrevPage")
								.attr("disabled", true);
						lst.removeClass("disLastPage").addClass("lastPage")
								.attr("disabled", false);
						nex.removeClass("disNextPage").addClass("nextPage")
								.attr("disabled", false);

					} else if (pginfo.currentPage == pginfo.pagesCount) {
						f1t.removeClass("disFirstPage").addClass("firstPage")
								.attr("disabled", false);
						pre.removeClass("disPrevPage").addClass("prevPage")
								.attr("disabled", false);
						lst.removeClass("lastPage").addClass("disLastPage")
								.attr("disabled", true);
						nex.removeClass("nextPage").addClass("disNextPage")
								.attr("disabled", true);
			...
			_resetPageBtns : function(pginfo) {
				...
				var _pagebar_div = $('#_pagebar_pagesdiv', elct);
				if (pginfo.pagesCount == 1 || pginfo.pagesCount == 0) {
			...
			_beforeAjax : function() {
				if (!$.isFunction(p.beforeAjax)) {
					var _d = $('#_centerMsg', elct);
					var _bd = $(document.body);
					_d.css({
								'top' : (_bd.scrollTop() + (_bd.height() - _d
										.height())
										/ 2),
								'left' : (_bd.scrollLeft() + (_bd.width() - _d
										.width())
										/ 2)
							});
					$('#showmsg', elct).width(_bd.width()).height(_bd.height())
							.css({
										'top' : 0,
										'left' : 0
									}).show();
			...
			_showLazyPagingDiv : function(pid, imgId, v, way) {
				var lazyPageDiv = $("#_lazyPagingDiv_" + pid, elct);
				var offset = $('#' + imgId, elct).offset();
				if (v) {
					var lazypaging_info = _lazypgInfoMap.get(pid);
					$('[id^=_lazyPagingDiv_]', elct).hide();
					$('#_lazy_page_count_' + pid, elct)
							.text(lazypaging_info.allCount);
					g._resetLazyPageBtns(pid);
					if (way == 1) {
						lazyPageDiv.css('top', offset.top);
						$('#_lazy_page_currentpage_' + pid, elct)
								.text(lazypaging_info.currentPage);

					} else {
						var lastcount = lazypaging_info.currentPage
								* lazypaging_info.pageSize;
					...
					lazyPageDiv.css('left', offset.left);
			...
			_createLazyPagingDiv : function(pid) {
				if ($("#_lazyPagingDiv_" + pid, eclt).size() < 1) {
					return true;

				}
				var lazyPagingDiv = document.createElement("div");
				$(lazyPagingDiv).attr('id', '_lazyPagingDiv_' + pid)
						.addClass("lazyPagediv").html(lzpagetabletemp.replace(
								/\$/g, pid)).mouseout(function(e) {
							g._showLazyPagingDiv(pid, 'null', false);
							e = e || window.event;
							var mousePos = mousePosition(e);
							g._d("lazypagex=" + lazypagex + ",lazypagey="
									+ lazypagey + ",lazypagex2=" + lazypagex2
									+ ",lazypagey2=" + lazypagey2);
							g._d("mousePos.x=" + mousePos.x + ",mousePos.y="
									+ mousePos.y);
							if (mousePos.x < lazypagex
									|| mousePos.y < lazypagey
									|| mousePos.x >= lazypagex2
			...
			_createMoreLazyPagingDiv : function(pid) {
				...
				var lazyPagingDiv = document.createElement("div");
				$(lazyPagingDiv).attr('id', '_lazyPagingDiv_' + pid)
						.addClass("lazyPagediv").html(lzmorepagetemp.replace(
								/\$/g, pid));
				document.body.appendChild(lazyPagingDiv);
				var lazypaging_info = _lazypgInfoMap.get(pid);
				$('#_lazy_page_count_' + pid, elct)
			...
			_initPageInfo : function() {
				if (_serverPagingMode == 'client') {
					...
					g._d('处理分页:pagingInfo.allCount=' + pagingInfo.allCount
							+ ",pagingInfo.pageSize=" + pagingInfo.pageSize
							+ ",pagingInfo.pagesCount=" + pagingInfo.pagesCount
			...
			_newPagingServerMakeTable : function(tree, msg) {
				...
				pagingInfo.pagesCount = Math.ceil(pagingInfo.allCount
						/ pagingInfo.pageSize * 1.0);
				if (!p.lazy)
					g._showTable(tree, 0, firstLevelNodes.length + 1);
				else
					g._showLazyTable(tree, p.allDataInfoWithPageInfo.data);
				g._resetBasePageBtns();
				if (p.exchangeColor)
				...
				if (p.onPagingSuccess)
					p.onPagingSuccess(elct);
			...
			_getColumnWidthArr : function() {
				$('div.gridCol[cmtitle=1]', elct).each(function() {
			...
			_addTitleHead : function() {
				...
				if (p.checkOption == 'radio' || p.checkOption == 'multi') {
					if (p.checkOption == 'multi' && p.allCheck) {
						checkAllButton = $("<input type='checkbox' style='width:20px;border:0px;' id='_checkAll'>")
								.click(function() {
											g._chooseAll();
										});
					}
				...
				if (p.rowCount && p._countColumnIndex == -2) {
					var jcdiv = $('<div class="gridCol"></div>')
							.html(p.countColumnDesc).width(p.countCellWd);
					$("<th>").attr('id', 'countCell').addClass('center')
							.addClass('countCell').width(p.countCellWd)
							.append(jcdiv).appendTo(newRow);
				...
				for (var ii = 0; ii < lenlen; ii++) {
					...
					// lsq
					if (p.columnModel[i].width != null
							&& p.columnModel[i].width != '*') {
						jcdiv.width(p.columnModel[i].width)
								.html(oneColumn.header);
						j_cell.width(p.columnModel[i].width);
					}
					i = i + 1;
					if (p._checkColumnIndex == -1
							&& p.checkColumnNm == oneColumn.headerIndex) {
						p._checkColumnIndex = ii;
					}
					if (p._countColumnIndex == -1
							&& p.countColumnNm == oneColumn.headerIndex) {
						p._countColumnIndex = ii;
					}
					if (p._expandColumnIndex == -1
							&& p.expandColumnNm == oneColumn.headerIndex) {
					...
					j_cell.addClass('center').attr('id', oneColumn.headerIndex)
							.append(jcdiv).appendTo(newRow);
			...
			_getFirstIndexInThisPage : function() {
				var _firstIndex = 0;
				if (p.pageBar)
					_firstIndex = (pagingInfo.currentPage - 1)
							* pagingInfo.pageSize;
				return _firstIndex;

			},
			_clearContent : function() {
				$('#' + p.tableId, elct).children().remove();
			},
			_makePageBar : function(div) {
				var msgCell = document.createElement("div");
				$(msgCell)
						.attr('id', '_pagebar_msgCell')
						.addClass('pages')
						.width(200)
						.html([
								'<span>每页</span><span><select ',
								' id="_changePageSizeSel"><option value="5">5</option><option value="10" selected>10</option>',
								'<option value="20">20</option><option value="40">40</option><option value="100">100</option></select></span><span>条</span>',
								"<span id='msgCell'>当前第",
								pagingInfo.currentPage, "页/总共",
								pagingInfo.pagesCount, "页</span>"].join(''))
						.appendTo(div);
				var pagesDiv = document.createElement("div");
				$(pagesDiv)
						.attr('id', '_pagebar_pagesdiv')
						.addClass('pagination2')
						.html('<ul> <li class="j-first disabled"> <a style="display: none;" class="first pgb" href="javascript:;"><span>首页</span></a> <span class="first"><span>首页</span></span> </li> <li class="j-prev disabled"> <a style="display: none;" class="previous pgb" href="javascript:;"><span>上一页</span></a> <span class="previous"><span>上一页</span></span> </li> <li class="j-next"> <a style="display: block;" class="next pgb" href="javascript:;"><span>下一页</span></a> <span style="display: none;" class="next"><span>下一页</span></span> </li> <li class="j-last"> <a style="display: block;" class="pgb last" href="javascript:;"><span>末页</span></a> <span style="display: none;" class="last"><span>末页</span></span> </li> <li class="jumpto"><input class="textInput" size="4" value="1" type="text"><input class="goto"  type="button"></li> </ul>');
				...
				if (pagingInfo.pagesCount != 0) {
					if (_serverPagingMode == 'client') {
						...
						pagingInfo.pagesCount = Math.ceil(pagingInfo.allCount
								/ pagingInfo.pageSize * 1.0);
						pagingInfo.currentPage = 1;
						$('#msgCell', elct).html("当前第" + pagingInfo.currentPage
								+ "页/总共" + pagingInfo.pagesCount + "页");
						g._toPage(1);
						$('#' + p.tableId, elct).focus();
			...
			// 懒加载的添加行对象
			_addOneLazyRowByData : function(index, rowData, level, nth, sp,
					_num) {
				var newRow = document.createElement('tr');
				...
				if (_isP != '1')
					lastLeafMap.put('level' + level + _parent, _id);
				...
				g._addOneLazyRowContent(rowData, newRow, _id, _isP, level,
						checkSpan, sp, countV);
			...
			_addOneRowByData : function(tableObj, index, rowData, level) {
				g
						._d(1463
								+ "----添加节点...  _addOneRowByData() rowData.- p.dynamicColumn="
								+ rowData[p.dynamicColumn])
				var newRow = tableObj.insertRow(index - 1);
				var _id = rowData[p.idColumn];
				var _parent = rowData[p.parentColumn];
				var _isP;
				if (g.superGrid) {
					_isP = rowData[p.dynamicColumn];
				} else {
					_isP = g.isParent(rowData);
				}
				...
				if (level == 1)
					newRow.setAttribute('_node_path', '_node-1');
				else {
				...
				if (p.rowCount)
					countV = g._getCountVal(rowData, newRow, index, level);
				g._addOneRowContent(rowData, newRow, _id, _isP, level,
						checkSpan, countV);
				g._addOneRowListerners(newRow);
				var _emptyTitleHead = document.createElement("td");
				$(_emptyTitleHead).addClass('emptyTh').appendTo(newRow);
			...
			_genrateExpandImg : function(level, _isP, _id, newRow, cdiv) {
				g._d(1506 + "----添加加减号图片-- _genrateExpandImg()...")
				var ct = level - 1;
				var ans = [];
			...
			_addOneRowContent : function(rowData, newRow, _id, _isP, level,
					checkSpan, countV) {
				g._d(1545 + "----添加节点具体内容-- _addOneRowContent()...")
				var i = 0;
				if (p._countColumnIndex == -2) {
					var ctCell = $('<td></td>').css('padding', '0')
							.width(p.countCellWd);
					var jdiv = $('<div></div>').width(p.countCellWd)
							.html(countV).appendTo(ctCell);
				...
				for (; i < headColumns.length; i++) {
					...
					$(newSmallCell).attr('_td_pro', headColumns[i]).css(
							'padding', '0').width(headColumnWidths[i]);
					var cdiv = $('<div></div>').width(headColumnWidths[i]);// lsq
					var _t = rowData[headColumns[i]];
					if (p._expandColumnIndex == i)
						g._genrateExpandImg(level, _isP, _id, newRow, cdiv);
					if (p._checkColumnIndex == i)
						cdiv.append(checkSpan);
					if (p._countColumnIndex == i)
						cdiv.append(countV);
					if (_t != '') {
						var showTypeDesc = p.columnModel[i].columntype;
						if (showTypeDesc != null) {
							cdiv.append(g._createContent(showTypeDesc, rowData,
									headColumns[i], _id, newRow).firstChild);
			...
			_addOneLazyRowContent : function(rowData, newRow, _id, _isP, level,
					checkSpan, sp, countV) {
				var i = 0;
				// 如果要添加序列所在的单独列.
				if (p._countColumnIndex == -2) {
				...
				for (; i < headColumns.length; i++) {
					...
					var _t = rowData[headColumns[i]];
					if (p._expandColumnIndex == i)
						g._genrateExpandImg2(level, _isP, _id, sp, cdiv);
					if (p._checkColumnIndex == i)
						cdiv.append(checkSpan);
					if (p._countColumnIndex == i)
						cdiv.append(countV);
			...
			_createOpenImg : function(id) {
				var _i = $(["<IMG id='img_", id, "' style='CURSOR: hand' ",
						" ' src='", p.openImg, "'/>"].join(''));
				_i.unbind('click');
			...
			_createLeafImg : function(id) {
				return $(["<IMG id='img_", id, "' src='", p.noparentImg, "'/>"]
						.join(''))[0];
			},
			_createCloseImg : function(id) {
				return $(["<IMG id='img_", id, "' style='CURSOR: hand' ",
						" ' src='", p.closeImg, "'/>"].join('')).click(
						function(e) {
							// var theEvent = window.event ||
							// arguments.callee.caller.arguments[0];
			...
			_addChild : function(pid, data) {
				var pNode = $('#_node' + pid, elct)[0];
				var img = $('#img_' + pid, elct)[0];
				p._dynamicAddByUse = 1;
			...
			_toLazyPage : function(pid, operCode) {
				...
				if (operCode == '_morepage')
					...
					temp = g._getCurrentPage(lazypaing_info, '', operCode);
				if (temp == -1)
					return;
				var param = {
					analyze : p.analyzeAtServer,
				...
				$.ajax({
							...
							success : function(msg) {
								...
								var parentNode = $('#_node' + pid, elct)[0];
								var img = $('#img_' + pid, eclt)[0];
								var o = new Date();
								if (operCode != '_morepage') {
									...
									var numstrs = $('[_node_parent=_node' + pid
											+ ']:last').attr('rownum')
											.split('.');
			...
			_resetCurrentPageSize : function() {
				var mcel = $('#msgCell', elct);
				mcel.html(["当前第", pagingInfo.currentPage, "页/总共",
			...
			_toPage : function(operCode) {
				...
				if (_serverPagingMode == 'client') {
					...
					g._showTable(document.getElementById(p.tableId),
			...
			_getCountVal : function(rowData, newRow, index, level) {
				...
				if (p.rowCountOption + '' == '3') {
					...
					if (findInArray(parents, _parent) != -1) {
						...
						indexNum = 1
								+ findInArray(firstLevelNodes, '_node' + _id);
						newRow.setAttribute('_node_num', indexNum);
						_idToNumMap.put('_node' + _id, indexNum);
			...
			_addCheckOptionCell : function(rowData, newRow) {
				...
				if (p.checkOption == 'radio') {
					...
					createRadio(checkCell, setOptionDisabled,
							'width:20px;border:0px;', '_chks', _id,
							p.handleCheck, '', '', '');
					return checkCell;
					...
					} else if (p.multiChooseMode == 9) { 
						createCheckbox(checkCell, setOptionDisabled,
								'width:20px;border:0px;', '_chks', _id,
								function() {
									g._chooseChildrenNode(this,_id); 
									g._cancleChildrenNode(this,_id);
									if (p.handleCheck)
										p.handleCheck(this);
			...
			seeChildren : function(nodeid) {
				if(!p.lazy)
					return parentToChildMap.get(nodeid);
				else if(p.multiChooseMode==9)//只在特殊情况下面进行处理查询子节点.
				{ 
					var ans = [];
					$('tr[_node_parent='+nodeid+']',elct).each(function(){
			...
			isParent : function(rowobj) {
				if (!p.lazy) {
					if (g.superGrid) {// 如果是超级表格.
						return rowobj[p.dynamicColumn];
					} else {
		...
		var lazypagex = 0, lazypagey = 0, lazypagex2 = 0, lazypagey2 = 0;
		// lsq
		var lzpagetabletemp = [
				'<table cellspacing=0 cellpadding=0  class="lazyPageTable"><tr><td>',
				'<button class=" lazypagebarbtn firstPage" onclick="g._toLazyPage(\'$\', \'first\')" id="_fstbtn_$" title="第一页"/></td>',
				'<td><button class=" lazypagebarbtn prevPage" onclick="g._toLazyPage(\'$\',\'pre\')" title="前一页" id="_prebtn_$"/></td>',
				'<td><button class=" lazypagebarbtn nextPage" id="_nxtbtn_$"  onclick="g._toLazyPage(\'$\',\'next\')" title="后一页"/></td>',
				'<td><button class=" lazypagebarbtn lastPage" onclick="g._toLazyPage(\'$\',\'last\')"  id="_lstbtn_$" title="最后一页"/></td>',
				'<td class="splitbar"><td  class="lazyPagingMsg" >总共<span id="_lazy_page_count_$" class="lazyPagingNum">',
				'</span>条 当前显示第<span id="_lazy_page_currentpage_$"	class="lazyPagingNum"></span>页</td></table>']
				.join('');
		var lzmorepagetemp = [
				'<table cellspacing=0 cellpadding=0  class="lazyPageTable"><tr>',
				'<td  class="lazyPagingMsg2" >当前层次总共<span id="_lazy_page_count_$" class="lazyPagingNum">',
				'</span>条 当前显示第<span class="lazyPagingNum">1</span> - <span id="_lazy_page_currentpage_$"	class="lazyPagingNum"></span>条</td></table>']
				.join('');
		...
		// 如果是静态加载并且设置了dynamicColumn,就强制不使用分页模式！
		if (p.data != null && p.dynamicColumn != '') {
			p.pageBar = false;
			g.superGrid = true;// 定义一个变量为超级表格!
		}
		...
		if (p.checkOption == 'radio' || p.checkOption == 'multi') {
			if (p.checkColumnNm == null)
				p._checkColumnIndex = 0;
			else
				p._checkColumnIndex = -1;

		}
		if (p.rowCount) {
			if (p.countColumnNm == null)
				p._countColumnIndex = -2;
			else
				p._countColumnIndex = -1;

		}

		if (p.expandColumnNm)
			p._expandColumnIndex = -1;
		else
			p._expandColumnIndex = 0;// 默认为0
		...
		if (p.dataUrl != null) {
			...
			if (p.lazy) {
				p.analyzeAtServer = false;
				p.rowCountOption = '3';
				if(p.multiChooseMode!=9 )//懒加载情况下面,如果不是自定义的，就全部设置为1.
					p.multiChooseMode = 1;
				p.expandAll = false;
				if (!p.lazyLoadUrl) {
		...
		// 设置最外层表格树的高度和宽度.
		g.allTreeDiv = $("<div style='display:none' id='allTreeDiv' class='gridTree'></div>")
				.height(_h).width(p.width);
		...
		g._title_table = $("<div id='_title_Div' class='gridThead'  ></div>")
				.append($('<div class="myhDivBox"></div>').append(g
						._addTitleHead()));
		g._title_Div = $("<div id='_header_Div' class='gridHeader'></div>")
				.append(g._title_table);
		g._content_div = $('<div id="_content_div" class="gridTbody"></div>')
				.height(p.height).scroll(function() {
							g.scroll();
						});
		g._scroller_div = $("<div id='_scroller_div' class='gridScroller'></div>")
				.height(p.height).append(g._content_div).scroll(function() {
							g.scroll();
						});
		g.allTreeDiv.append(g._tool_Div).append(g._title_Div)
				.append(g._scroller_div);
		if (p.pageBar) {
		...
		elct.innerHTML = '';
		// 添加等待条
		$('<div class="showprogress" id="showmsg"><div id="_centerMsg" class="centerMsg" align="top"><img src="'
				+ p.lazyLoadImg + '">正在加载,请稍候!</div></div>').appendTo(elct);
	...
	function _checkedAll(checkName, v) {
		$('[name=' + checkName + "][userSetDisabled!=disabled]").attr(
				'checked', v);
